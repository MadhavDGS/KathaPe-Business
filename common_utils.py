...
# Initialize database schema for pending payments
def init_pending_payments_schema():
    """Initialize the pending_payments table and related schema updates"""
    schema_sql = """
    -- Create pending_payments table for PhonePe payment approval system
    ...
    try:
        ...
        if conn:
            with conn.cursor() as cursor:
                ...
                conn.commit()
            release_db_connection(conn)
            logger.info("Pending payments schema initialized successfully")
            return True
    except Exception as e:
        logger.error(f"Error initializing pending payments schema: {str(e)}")
        return False
